...
import com.seabreeze.robot.base.ext.tool.DensityUtil;
...
public class SmoothCheckBox extends View implements Checkable {
    ...
    private void init(AttributeSet attrs) {
        ...
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_stroke_width, DensityUtil.dp2px(0));
        ta.recycle();
    ...
    private int measureSize(int measureSpec) {
        int defSize = DensityUtil.dp2px(DEF_DRAW_SIZE);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int paddingLeft = getPaddingLeft();//28
        int paddingTop = getPaddingTop();
        int paddingRight = getPaddingRight();
        int paddingBottom = getPaddingBottom();

        int width = measureSize(widthMeasureSpec) + paddingLeft + paddingRight;//97
        int height = measureSize(heightMeasureSpec) + paddingTop + paddingBottom;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int viewWidth = getMeasuredWidth() - paddingLeft - paddingRight;
        int viewHeight = getMeasuredHeight() - paddingTop - paddingBottom;
        mStrokeWidth = (mStrokeWidth == 0 ? viewWidth / 10 : mStrokeWidth);
        mStrokeWidth = mStrokeWidth > viewWidth / 5 ? viewWidth / 5 : mStrokeWidth;
        mStrokeWidth = (mStrokeWidth < 3) ? 3 : mStrokeWidth;
        mCenterPoint.x = viewWidth / 2;
        mCenterPoint.y = viewHeight / 2;

        mTickPoints[0].x = Math.round((float) (getMeasuredWidth() - paddingLeft - paddingRight) / 30 * 7);
        mTickPoints[0].y = Math.round((float) (getMeasuredHeight() - paddingTop - paddingBottom) / 30 * 14);
        mTickPoints[1].x = Math.round((float) (getMeasuredWidth() - paddingLeft - paddingRight) / 30 * 13);
        mTickPoints[1].y = Math.round((float) (getMeasuredHeight() - paddingTop - paddingBottom) / 30 * 20);
        mTickPoints[2].x = Math.round((float) (getMeasuredWidth() - paddingLeft - paddingRight) / 30 * 22);
        mTickPoints[2].y = Math.round((float) (getMeasuredHeight() - paddingTop - paddingBottom) / 30 * 10);
    ...
    private void drawCenter(Canvas canvas) {
        ...
        canvas.drawCircle(mCenterPoint.x + getPaddingLeft(), mCenterPoint.y + getPaddingTop(), radius, mPaint);
    }

    private void drawBorder(Canvas canvas) {
        mFloorPaint.setColor(mFloorColor);
        int radius = mCenterPoint.x;
        canvas.drawCircle(mCenterPoint.x + getPaddingLeft(), mCenterPoint.y + getPaddingTop(), radius * mFloorScale, mFloorPaint);
    }
    ...
    private void drawTickPath(Canvas canvas) {
        ...
        if (mDrewDistance < mLeftLineDistance) {
            ...
            mTickPath.offset(paddingLeft, paddingTop);
